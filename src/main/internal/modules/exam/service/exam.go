...
// NewExamService creates service with injected repository
...
// NewExamServiceWithDeps creates service with injected dependencies (for DI)
func NewExamServiceWithDeps(repo *repository.ExamRepository) *ExamService {
	return &ExamService{examRepository: repo}
}
...
func (e *ExamService) GetAllExam(c *gin.Context, request pagination.Request[map[string]interface{}]) *database.Paginator {
	...
	// Filter by school_code untuk multi-tenant isolation
	...
	paging := database.NewPagination[map[string]interface{}](e.examRepository.Database).
		SetModel([]school.Exam{}).
		SetPreloads(
			"DetailSubject",
			"ExamMember",
...
func (e *ExamService) CreateNewExam(c *gin.Context, request exam_request.ModifyExamRequest) *school.Exam {
	...
	data := &school.Exam{
		...
		SchoolCode:     claims.SchoolCode, // Set SchoolCode dari JWT
	}
...
func (e *ExamService) CreateExamQuestion(claims jwt.Claims, request exam_request.ModifyExamQuestionRequest) exam_request.ModifyExamQuestionRequest {
	...
	if err := tx.Create(dataQuestion).Error; err != nil {
		tx.Rollback()
		log.Error().Msg("Failed to create exam question")
	...
	if err := tx.Create(&options).Error; err != nil {
		tx.Rollback()
		log.Error().Msg("Failed to create exam question Options")
...
func (e *ExamService) UpdateExamQuestion(id uint, request exam_request.ModifyExamQuestionRequest) exam_request.ModifyExamQuestionRequest {
	...
	// 7. Commit transaksi
	if err := tx.Commit().Error; err != nil {
		log.Error().Err(err).Msg("Gagal commit saat update exam question")
		panic(exception.NewBadRequestExceptionStruct(response.BadRequest, "failed to update exam question"))
	}
...
func (e *ExamService) UploadQuestion(c *gin.Context) {
	...
	if err != nil {
		response.InternalError(c, "Failed Upload Question", err)
		return
	}

	if ext := strings.ToLower(filepath.Ext(file.Filename)); ext != ".docx" {
		response.InternalError(c, "Failed Upload Question, Format file must be .docx", err)
		return
	}
	...
	if err != nil {
		response.InternalError(c, "Failed read file", err)
		return
	}
	...
	for i, row := range result {
		...
		if err := tx.Create(&examQuestion).Error; err != nil {
			tx.Rollback()
			response.InternalError(c, fmt.Sprintf("Gagal simpan data di baris %d", i+2), err)
			break
		}
	...
	response.OK(c, "Success Upload Question", nil)
}
...
func (e *ExamService) uploadQuestion(c *gin.Context, typeQuestion string, err error, fileBytes []byte, file *multipart.FileHeader) ([]parsedocx.ResultParse, bool) {
	...
	if typeQuestion == "ESSAY" {
		...
		if err != nil {
			response.InternalError(c, "Failed to parse docx", err)
			return nil, false
		}
	} else {
...
func (e *ExamService) GetAllBankQuestion(c *gin.Context, request pagination.Request[map[string]interface{}]) *database.Paginator {
	...
	paging := database.NewPagination[map[string]interface{}](e.examRepository.Database).
		SetModel([]view.MasterBankQuestionResponse{}).
		SetRequest(&request).
		FindAllPaging()
	return paging
}
